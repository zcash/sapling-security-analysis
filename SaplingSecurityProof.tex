...
a \emph{valid transaction input} if $\vbal =  \sum \val_i - \sum \outval_j$.
...
\begin{enumerate}
 \item We choose uniform $\sk = (\ask,\nsk)$; and give $\pak=\pak(\sk)$ to \adv.
\item Let $\ivk \defeq \IVK(\pak)$. \adv outputs a ledger \ledger, a note $\note$, a valid transaction input $(\inputs=(\note),\outputs,\vbal)$,
...
\item For each $i\in [s]$, $\output_i$ belongs to $\ivk_i$.
 ...
 \item For some $i\in [\ell]$, $\dec(\out_i,\ivk_i)$ does not return $\output_i$.
\end{enumerate}
\end{enumerate}
