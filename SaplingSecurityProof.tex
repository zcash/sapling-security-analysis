...
\begin{itemize}
 ...
 \item $\sum_{j=1}^{\ell} \val_j-\sum_{j=\ell+1}^{s} \val_j=0$.
...
\begin{enumerate}
 \item Choose random $\rcms,\rcvs\in \F^s$.
...
\end{claim}
\begin{proof}
The output of \makerandomizedtx is of the form $(\inps,\outs,\sigs)$.
...
We show that except w.p. \negl, any corresponding elements in $\inp_j$ and $\inp'_j$ are distributed identically conditioned on fixing to an identical value of the previous elements.
%We also show for each element that its distribution is independent of that of the same element in $D'$
 ...
 \begin{itemize}
  ...
  \item $\rt=\rt'$.
  \item $\rk=\ak + \alpha\cdot \g$, $\rk' = \alpha' + \cdot \g$. Are independent and both uniform in \G because of the uniform choice of $\alpha$ in \makerandomizedtx.
  ...
  \item $\prf,\prf'$ - Assuming  $(\nf,\rt,\rk,\cv)=(\nf',\rt',\rk',\cv')$, it follows from the witness indistinguishability of the SNARK
  a random oracle value, depending only on the randomness of \NF. It is independent of $\nf'$ which is a value of 
  
  $\NF$ on a distinct input because $\pos_j\neq \pos'_j$.
  
  
 \nk is common randomness, if NF random oracle then \nf s are uniform and independent.
