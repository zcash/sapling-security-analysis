...
if for each $j\in [\ell]$ $\input_j$ contains a path \jof{\pak} from $\notecom(\note_j)$ to \rt;
and
\[\sum_{j=1}^{\ell} \val_j-\sum_{j=\ell+1}^{s} \val_j=0\]
\noindent
...
\begin{enumerate}
 \item Choose random $ \alphas \leftarrow \F^{\ell},\rcms,\rcvs\in \F^s$.
...
Assume \NF, \KDF are random oracles, and that \symencrypt{K}{m} produces a uniform output when $K$ is uniform and $m$ is fixed.

Then
\begin{enumerate}
 \item 
the distribution of $\makerandomizedtx(\rt,\vbal,\inputs,\outputs)$ depends only on $(\rt,\vbal)$.
\item For any $(\inputs',\outputs')$ with $|\inputs'|=\ell,|\outputs'|=s$ that is also consistent with $\rt,\vbal$, such that the notes in $\inputs'$ are disjoint from those in \inputs;	
the distribution of

$\makerandomizedtx(\rt,\vbal,\inputs',\outputs')$, over the inner randomness of \makerandomizedtx, the SNARK prover, and that of the random oracles \NF,\KDF, is independent from 
$\makerandomizedtx(\rt,\vbal,\inputs,\outputs)$.
