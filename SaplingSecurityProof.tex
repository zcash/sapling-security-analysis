...
\begin{enumerate}
 \item 
$\NF(\nk,\cm,\pos)= \RO(\nk,\MPH(\cm,\pos))$ where \RO is a random oracle and \MPH is a collision-resistant function.
\item
...
\item The notes in \inputs and $\inputs'$ are all distinct.
\item $(\inputs,\outputs)$ and $(\inputs',\outputs')$ are both consistent with $\rt,\vbal$
\item The distributions $D\defeq \makerandomizedtx(\rt,\vbal,\inputs,\outputs)$ and $D'\defeq \makerand\makerandomizedtx(\rt,\vbal,\inputs',\outputs')$
\end{itemize}
is \negl.
...
We show that the distribution of each element in $\inp_j$, conditioned on the previous elements depends only on the value of the previous elements together with $\rt,\vbal$.
We also show for each element that its distribution is independent of that of the same element in $D'$
 
 Such an element $\inp=(\nf,\rt,\rk,\cv,\prf)$
 \begin{itemize}
  \item  $\nf = \NF(\nk,\cm,\pos)$. This is a random oracle value, depending only on the randomness of \NF. It is independent of $\nf'$ which is a value of 
