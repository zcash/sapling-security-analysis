...
A noteable property of the protocol is a separation of proving and signing authority. A ``delegated spender/prover'' creates transaction with the help
...
\begin{enumerate}
 ...
 \item \textbf{Spendability}: An adversary should not be able to send the honest party a note that was successfully received, but cannot be later spent.
\end{enumerate}
...
we state the following theorem that is almost implicit in \cite{schnorrSecurity}
...
We thank Matthew D. Green for conversations on simulation extractability that inspired in particular the definition and use of invertible group samplers.
\bibliography{refs}
