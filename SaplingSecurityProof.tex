...
\noindent
\underline{\makerandomizedtx(\inputs,\outputs,\vbal)}
...
For $i\in [n]$ let us denote respectively by $\inpnotes_i,\outnotes_i$ the positioned input and output notes in $\tx_i$.


Let us denote by $\bal(\tx)$ the sum of note values in input notes of \tx minus the sum of values in output notes of \tx.
And by $\rand(\tx)$ the sum of values $\rcv$ in input note witnesses of \tx minus the sum of values \rcv in output note witnesses of \tx.
...
\begin{enumerate}
\item  \adv' first checks if  for some $i\in [n]$ and $\inp\in \tx_i, \posnote(\inp)\notin \outnotes_{<i}$.
